/*-------------------------------UNCLASSIFIED---------------------------------*/
// File: iccFIR.c
// Desc: This file contains the code for the iccFIR.
// Hist: When       Who  What
...
//------------------------------- COMMON-CODE FLT32         START.
  ...
  FLT32 tt;
  INT32 ii;
  iccFIRFilterSpecFLT32 *spec=NULL;

  // Malloc the memory for the spec.
  //----------------------------------------------------------------------------
  spec = (iccFIRFilterSpecFLT32 *) iccMallocVOID_Align032(sizeof(iccFIRFilterSpecFLT32));

  // Initialize and allocate.
  //----------------------------------------------------------------------------
  spec->nTaps = nTaps;
  spec->nHist = nTaps-1;
  spec->fw    = iccMallocFLT32_Align032( spec->nTaps );
  spec->fd    = iccMallocFLT32_Align032( spec->nTaps );
  spec->fh    = iccMallocFLT32_Align032( spec->nHist*2 );
  ...
  //iccBlackmanWindowFLT32( spec->fw, 1, spec->fw, 1, nTaps );
...
//------------------------------- COMMON-CODE FLT64         START.
  ...
  FLT64 tt;
  INT32 ii;
  iccFIRFilterSpecFLT64 *spec=NULL;

  // Malloc the memory for the spec.
  //----------------------------------------------------------------------------
  spec = (iccFIRFilterSpecFLT64 *) iccMallocVOID_Align032(sizeof(iccFIRFilterSpecFLT64));

  // Initialize and allocate.
  //----------------------------------------------------------------------------
  spec->nTaps = nTaps;
  spec->nHist = nTaps-1;
  spec->fw    = iccMallocFLT64_Align032( spec->nTaps );
  spec->fd    = iccMallocFLT64_Align032( spec->nTaps );
  spec->fh    = iccMallocFLT64_Align032( spec->nHist*2 );
  ...
  //iccBlackmanWindowFLT64( spec->fw, 1, spec->fw, 1, nTaps );
...
//------------------------------- COMMON-CODE FLT96         START.
  ...
  FLT96 tt;
  INT32 ii;
  iccFIRFilterSpecFLT96 *spec=NULL;

  // Malloc the memory for the spec.
  //----------------------------------------------------------------------------
  spec = (iccFIRFilterSpecFLT96 *) iccMallocVOID_Align032(sizeof(iccFIRFilterSpecFLT96));

  // Initialize and allocate.
  //----------------------------------------------------------------------------
  spec->nTaps = nTaps;
  spec->nHist = nTaps-1;
  spec->fw    = iccMallocFLT96_Align032( spec->nTaps );
  spec->fd    = iccMallocFLT96_Align032( spec->nTaps );
  spec->fh    = iccMallocFLT96_Align032( spec->nHist*2 );
  ...
  //iccBlackmanWindowFLT96( spec->fw, 1, spec->fw, 1, nTaps );
...
PERL_PREPROCESS  //----------------------------------------------------------------------------
PERL_PREPROCESS  fwt = iccMallocXXXXX_Align032( spec->nTaps );
PERL_PREPROCESS  
PERL_PREPROCESS  // Generate the filter weights.
PERL_PREPROCESS  //----------------------------------------------------------------------------
PERL_PREPROCESS  for(ii =0; ii < nTaps; ii++)
...
//------------------------------- COMMON-CODE FLT32         START.
  ...
  //----------------------------------------------------------------------------
  fwt = iccMallocFLT32_Align032( spec->nTaps );
...
//------------------------------- COMMON-CODE FLT64         START.
  ...
  // Temporary filter weights.
  //----------------------------------------------------------------------------
  fwt = iccMallocFLT64_Align032( spec->nTaps );
...
//------------------------------- COMMON-CODE FLT96         START.
  ...
  // Temporary filter weights.
  //----------------------------------------------------------------------------
  fwt = iccMallocFLT96_Align032( spec->nTaps );
...
PERL_PREPROCESS    //icc_DEBUG("ii = %04ld  ", ii);
PERL_PREPROCESS    //icc_DEBUG("spec->nHist+ii = %04ld  ", spec->nHist+ii);
PERL_PREPROCESS    //icc_DEBUG("ii*ski = %04ld\n",ii*ski);
PERL_PREPROCESS  }
PERL_PREPROCESS
...
PERL_PREPROCESS  for(ii = 0; ii < spec->nHist; ii++)
PERL_PREPROCESS  {
PERL_PREPROCESS    dataO[ii*skj] = iccvConvolveXXXXX( &spec->fh[ii], 1, &spec->fw[0], 1, spec->nTaps );
PERL_PREPROCESS    //icc_DEBUG("ii = %04ld  ", ii);
PERL_PREPROCESS    //icc_DEBUG("ii*skj = %04ld\n",ii*ski);
PERL_PREPROCESS  }
PERL_PREPROCESS
...
PERL_PREPROCESS  {
PERL_PREPROCESS    dataO[ii*skj] = iccvConvolveXXXXX( &dataI[jj*ski], ski, &spec->fw[0], 1, spec->nTaps );// 1 to ski
PERL_PREPROCESS    //icc_DEBUG("ii = %04ld  ", ii);
PERL_PREPROCESS    //icc_DEBUG("jj = %04ld  ", jj);
PERL_PREPROCESS    //icc_DEBUG("ii*skj = %04ld  ", ii*skj);
PERL_PREPROCESS    //icc_DEBUG("jj*ski = %04ld\n",jj*ski);
PERL_PREPROCESS    ii++;
PERL_PREPROCESS  }
PERL_PREPROCESS 
...
PERL_PREPROCESS    //icc_DEBUG("ii = %04ld  ", ii);
PERL_PREPROCESS    //icc_DEBUG("n  = %04ld  ", n);
PERL_PREPROCESS    //icc_DEBUG("spec->nHist  = %04ld  ", spec->nHist);
PERL_PREPROCESS    //icc_DEBUG("(n-spec->nHist+ii)*ski = %04ld\n",(n-spec->nHist+ii)*ski);
PERL_PREPROCESS  }
#endif

//------------------------------------------------------------------------------
// Rout: iccFIRFilterFLT32
...
//------------------------------- COMMON-CODE FLT32          START.
  ...
  for(ii = 0; ii < spec->nHist; ii++)
  {
    ...
    //icc_DEBUG("ii = %04ld  ", ii);
    //icc_DEBUG("spec->nHist+ii = %04ld  ", spec->nHist+ii);
    //icc_DEBUG("ii*ski = %04ld\n",ii*ski);
  }
  ...
  for(ii = 0; ii < spec->nHist; ii++)
  {
    dataO[ii*skj] = iccvConvolveFLT32( &spec->fh[ii], 1, &spec->fw[0], 1, spec->nTaps );
    //icc_DEBUG("ii = %04ld  ", ii);
    //icc_DEBUG("ii*skj = %04ld\n",ii*ski);
  }
  ...
  {
    dataO[ii*skj] = iccvConvolveFLT32( &dataI[jj*ski], ski, &spec->fw[0], 1, spec->nTaps );// 1 to ski
    //icc_DEBUG("ii = %04ld  ", ii);
    //icc_DEBUG("jj = %04ld  ", jj);
    //icc_DEBUG("ii*skj = %04ld  ", ii*skj);
    //icc_DEBUG("jj*ski = %04ld\n",jj*ski);
    ii++;
  }
  ...
  for(ii = 0; ii < spec->nHist; ii++)
  {
    ...
    //icc_DEBUG("ii = %04ld  ", ii);
    //icc_DEBUG("n  = %04ld  ", n);
    //icc_DEBUG("spec->nHist  = %04ld  ", spec->nHist);
    //icc_DEBUG("(n-spec->nHist+ii)*ski = %04ld\n",(n-spec->nHist+ii)*ski);
  }
//------------------------------- COMMON-CODE FLT32           STOP.
}

//------------------------------------------------------------------------------
// Rout: iccFIRFilterFLT64
// Desc: This routine performs an FIR filter based on the spec.
//------------------------------------------------------------------------------
...
//------------------------------- COMMON-CODE FLT64          START.
  ...
  for(ii = 0; ii < spec->nHist; ii++)
  {
    dataO[ii*skj] = iccvConvolveFLT64( &spec->fh[ii], 1, &spec->fw[0], 1, spec->nTaps );
    //icc_DEBUG("ii = %04ld  ", ii);
    //icc_DEBUG("ii*skj = %04ld\n",ii*ski);
  }
  ...
  for(jj = 0; jj < (n-spec->nTaps+1); jj++)
  {
    dataO[ii*skj] = iccvConvolveFLT64( &dataI[jj*ski], ski, &spec->fw[0], 1, spec->nTaps );// 1 to ski
    //icc_DEBUG("ii = %04ld  ", ii);
    //icc_DEBUG("jj = %04ld  ", jj);
    //icc_DEBUG("ii*skj = %04ld  ", ii*skj);
    //icc_DEBUG("jj*ski = %04ld\n",jj*ski);
...
//------------------------------- COMMON-CODE FLT96          START.
  ...
  for(ii = 0; ii < spec->nHist; ii++)
  {
    dataO[ii*skj] = iccvConvolveFLT96( &spec->fh[ii], 1, &spec->fw[0], 1, spec->nTaps );
    //icc_DEBUG("ii = %04ld  ", ii);
    //icc_DEBUG("ii*skj = %04ld\n",ii*ski);
  }
  ...
  for(jj = 0; jj < (n-spec->nTaps+1); jj++)
  {
    dataO[ii*skj] = iccvConvolveFLT96( &dataI[jj*ski], ski, &spec->fw[0], 1, spec->nTaps );// 1 to ski
    //icc_DEBUG("ii = %04ld  ", ii);
    //icc_DEBUG("jj = %04ld  ", jj);
    //icc_DEBUG("ii*skj = %04ld  ", ii*skj);
    //icc_DEBUG("jj*ski = %04ld\n",jj*ski);
