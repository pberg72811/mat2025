/*----------------------------------------UNCLASSIFIED-----------------------------------------*/
// File: iccDatagramSocketSend.c
// Desc: This file contains the code for the iccDatagramSocketSend routine.
// Hist: When       Who  What
...
/*---------------------------------------------------------------------------------------------*/
//-----------------------------------------------------------------------------------------------
// Rout: iccDatagramSocketSendInit
...
//-----------------------------------------------------------------------------------------------
  spec->msg = iccMallocINT08_Align032(ICC_MAX_MSG_LEN);
  //for(ii=0; ii<ICC_MAX_MSG_LEN; ii++) spec->msg[ii] =0;
...
//-----------------------------------------------------------------------------------------------
  ...
  {
    perror("iccDatagramSocketSendInit:");
...
// Check message size.
//-----------------------------------------------------------------------------------------------
  if( msgSize > ICC_MAX_MSG_LEN)
  {
    XMC_ERROR_STDOUT("msgSize to greater then ICC_MAX_MSG_LEN (%d).\n",ICC_MAX_MSG_LEN);
  }
