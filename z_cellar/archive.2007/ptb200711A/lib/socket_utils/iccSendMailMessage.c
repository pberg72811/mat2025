/*-------------------------------UNCLASSIFIED---------------------------------*/
// File: iccSendMailMessage.c
// Desc: This file contains the code for the iccSendMailMessage routine.
// Hist: When       Who  What
...
INT32 iccSendMailMessage(CHR08 *message, CHR08 *fromWhom, CHR08 *fromWhere, CHR08 *toWhom, CHR08 *toWhere)
{
  ...
  // Open sockets.
  //----------------------------------------------------------------------------
  socketFDS = iccStreamSocketOpen( toWhere, ICC_SEND_MAIL_PORT);
  ...
  // Send the message.
  //----------------------------------------------------------------------------
  numBytes = iccStreamSocketSend(socketFDS, theMessage, strlen(theMessage));
  ...
  //----------------------------------------------------------------------------
  while ( (numBytes = iccStreamSocketReceive(socketFDS, theResponse, 1024 )) > 0)
  {
    //check somethings.
  };
