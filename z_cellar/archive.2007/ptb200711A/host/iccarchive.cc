...
//                              V                                                               -
//     :-----------:    :---------------:     :---------:                                       -
//     | FILE/PIPE |--->| iccarchive.cc |---> | Archive |                                       -
//     :-----------:    :---------------:     :---------:-:                                     -
//                              |               | Archive |                                     -
//                              V               :---------:                                     -
//                        Debug Statements                                                      -
...
// Examples:                                                                                    -
// pipe on                                                                                      -
// iccpicrecord/cint16/replay=2 _stream(ps=32M) 10e6 PIC1_A2D_TUN1I 1e6 16 2                    -
// iccarchive _stream pictest 1024*1024*1024 /data12/steswd/ 10                                 -
// pipe off                                                                                     -
//                                                                                              -
// results PICFLAGS "ipaddr1=192.168.0.9|join1=224.2.2.2|ipdisc=224.2.2.2|ioc=iix|rxrawsdds"    -
// pipe on                                                                                      -
// iccpicrecord/replay=2/SI/flags=^PICFLAGS _stream(ps=32M) 1024*1024*4 10e6 pic1_sdds12a       -
// iccarchive _stream pictest1 1024*1024*1024*8 /data11/steswd/ 10                              -
// pipe off                                                                                     -
//                                                                                              -
// pipe on                                                                                      -
// iccsddsrecord/ts=1080*1024*4  _stream(ps=32M) "192.168.0.30" "224.3.3.3" 5000                -
// iccarchive _stream pictest2 1024*1024*1024*8 /data11/steswd/ 10                              -
// pipe off                                                                                     -
//                                                                                              -
// results PICFLAGS "ipaddr1=192.168.0.9|join1=224.2.2.2|ipdisc=224.2.2.2|ioc=iix|rxrawsdds"    -
// pipe on                                                                                      -
// iccpicrecord/Si/dmabytes=1024*1080*4/flags=^PICFLAGS _stream(ps=128M) 10e6 pic1_sdds12a      -
// iccarchive/tl=1024*1024*32 _stream(ps=128M) pictest1 1024*1024*1024 /data14/steswd/ 25       -
// pipe off                                                                                     -
//                                                                                              -
...
VOID mainroutine()
{
  ...
  iccArchiveSpec *archiveSpec[NDIR];
  UNT64 nBatches;
  ...
  // Initialize the archive. (iccArchiveInit).
  //---------------------------------------------------------------------------------------------
  for(ii=0; ii<nDires; ii++)
  {
    sprintf(pathName,"%s%s", oDireName[ii].c_str(), oFileName.c_str());
    archiveSpec[ii] = iccArchiveInit(pathName, "tmp", nFiles[ii] );
  }

  // Reestablish the archive(s). (iccArchiveReestablish).
  //---------------------------------------------------------------------------------------------
  for(ii=0; ii<nDires; ii++) 
  {
    iccArchiveReestablish(archiveSpec[ii]);
  }
  ...
  timeTag = iccCurrentJ1970Seconds();

  // Start the processing loop.
  //---------------------------------------------------------------------------------------------
  while( !Mc->break_ )
  {
    // Generate an archive file name "theArchiveName". (iccArchiveProcess).
    //-------------------------------------------------------------------------------------------
    p2String = iccArchiveProcess( archiveSpec[archDex] );
    ...
    if( !realTimeFlag )
    {
      oHeader.timecode  = timeTag + 
	                  ...
	                  (icc_JULIAN_SECONDS_JAN1_1970 - icc_JULIAN_SECONDS_JAN1_1950);
    }
    else
    {
    ...
    {
      iccArchiveReport( archiveSpec[archDex] );
      archDex++;
