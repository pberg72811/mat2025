...
//                                                                                              -
// Examples:                                                                                    -
// iccrtgnoise noisefile cb 1024*1024 1024*1024*4 5 8.5e6                                       -
// iccrtgnoise/CONTINUOUS/ts=1024*1024*128 noisefile cb 1024 1024*1024*4 5 8.5e6                -
...
// iccrtgnoise/continuous/tl=2M _s2(ps=4M) si 1024*1024*1 1024*1024*4 20 8.5e6                  -
// fcalc/tl=2M _s3(ps=4M) _s1(ps=4M) _s2(ps=4M) +                                               -
// iccarchive/tl=2M _s3(ps=4M) src 1024*1024*128 /data11/steswd/ 10                             -
// pipe off                                                                                     -
//01.0 Description                                                                              -
//02.0 Requirements                                                                             -
//03.0 Interfaces                                                                               -
...
VOID mainroutine()
{
  ...
  if     ( oHeader.format == "SB" ) {
    ...
    nBufferINT08  = iccMallocINT08_Align032(nBufferSize*oHeader.bpe);
    if(uniformFlag) iccvRandINT08 (&seedUNT64, std, (INT08 *)nBufferINT08, 1, nBufferSize);
    else            iccvRandNINT08(&seedUNT64, std, (INT08 *)nBufferINT08, 1, nBufferSize);
  }
  else if( oHeader.format == "SI" ) {
    complexFlag   = 1;
    generic_rcopy = rcopyINT16;
    nBufferINT08  = iccMallocINT08_Align032(nBufferSize*oHeader.bpe);
    if(uniformFlag) iccvRandINT16 (&seedUNT64, std, (INT16 *)nBufferINT08, 1, nBufferSize);
    else            iccvRandNINT16(&seedUNT64, std, (INT16 *)nBufferINT08, 1, nBufferSize);
  }
  else if( oHeader.format == "SL" ) {
    complexFlag   = 1;
    generic_rcopy = rcopyINT32;
    nBufferINT08  = iccMallocINT08_Align032(nBufferSize*oHeader.bpe);
    if(uniformFlag) iccvRandINT32 (&seedUNT64, std, (INT32 *)nBufferINT08, 1, nBufferSize);
    else            iccvRandNINT32(&seedUNT64, std, (INT32 *)nBufferINT08, 1, nBufferSize);
  }
  else if( oHeader.format == "SF" ) {
    complexFlag   = 1;
    generic_rcopy = rcopyINT32;
    nBufferINT08  = iccMallocINT08_Align032(nBufferSize*oHeader.bpe);
    if(uniformFlag) iccvRandFLT32 (&seedUNT64, std, (FLT32 *)nBufferINT08, 1, nBufferSize);
    else            iccvRandNFLT32(&seedUNT64, std, (FLT32 *)nBufferINT08, 1, nBufferSize);
  }
  else if( oHeader.format == "SD" ) {
    complexFlag   = 1;
    generic_rcopy = rcopyINT64;
    nBufferINT08  = iccMallocINT08_Align032(nBufferSize*oHeader.bpe);
    if(uniformFlag) iccvRandFLT64 (&seedUNT64, std, (FLT64 *)nBufferINT08, 1, nBufferSize);
    else            iccvRandNFLT64(&seedUNT64, std, (FLT64 *)nBufferINT08, 1, nBufferSize);
  }
  else if( oHeader.format == "CB" ) {
    complexFlag   = 2;
    generic_rcopy = rcopyINT08;
    nBufferINT08  = iccMallocINT08_Align032(nBufferSize*oHeader.bpe);
    if(uniformFlag) iccvRandINT08 (&seedUNT64, std, (INT08 *)nBufferINT08, 1, nBufferSize*2);
    else            iccvRandNINT08(&seedUNT64, std, (INT08 *)nBufferINT08, 1, nBufferSize*2);
  }
  else if( oHeader.format == "CI" ) {
    complexFlag   = 2;
    generic_rcopy = rcopyINT16;
    nBufferINT08  = iccMallocINT08_Align032(nBufferSize*oHeader.bpe);
    if(uniformFlag) iccvRandINT16 (&seedUNT64, std, (INT16 *)nBufferINT08, 1, nBufferSize*2);
    else            iccvRandNINT16(&seedUNT64, std, (INT16 *)nBufferINT08, 1, nBufferSize*2);
  }
  else if( oHeader.format == "CL" ) {
    complexFlag   = 2;
    generic_rcopy = rcopyINT32;
    nBufferINT08  = iccMallocINT08_Align032(nBufferSize*oHeader.bpe);
    if(uniformFlag) iccvRandINT32 (&seedUNT64, std, (INT32 *)nBufferINT08, 1, nBufferSize*2);
    else            iccvRandNINT32(&seedUNT64, std, (INT32 *)nBufferINT08, 1, nBufferSize*2);
  }
  else if( oHeader.format == "CF" ) {
    complexFlag   = 2;
    generic_rcopy = rcopyINT32;
    nBufferINT08  = iccMallocINT08_Align032(nBufferSize*oHeader.bpe);
    if(uniformFlag) iccvRandFLT32 (&seedUNT64, std, (FLT32 *)nBufferINT08, 1, nBufferSize*2);
    else            iccvRandNFLT32(&seedUNT64, std, (FLT32 *)nBufferINT08, 1, nBufferSize*2);
  }
  else if( oHeader.format == "CD" ) {
    complexFlag   = 2;
    generic_rcopy = rcopyINT64;
    nBufferINT08  = iccMallocINT08_Align032(nBufferSize*oHeader.bpe);
    if(uniformFlag) iccvRandFLT64 (&seedUNT64, std, (FLT64 *)nBufferINT08, 1, nBufferSize*2);
    else            iccvRandNFLT64(&seedUNT64, std, (FLT64 *)nBufferINT08, 1, nBufferSize*2);
  }
  else {
    complexFlag   = 2;
    generic_rcopy = rcopyINT32;
  ...
  //-------------------------------------------------------------------------------
  XMC_DEBUG("Initialize iccrtnplay.\n");
  oDataINT08 = iccMallocINT08_Align032(xferLngth);
