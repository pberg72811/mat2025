...
//                             V                                                                -
//     :---------:     :-----------------:     :-----------:                                    -
//     | Archive |---> | iccdearchive.cc |---> | FILE/PIPE |                                    -
//     :---------:-:   :-----------------:     :-----------:                                    -
//       | Archive |           |                                                                -
//       :---------:           V                                                                -
//                        Debug Statements                                                      -
...
// Examples:                                                                                    -
// pipe on                                                                                      -
// ICCDEARCHIVE/tl=1080*1024 test1 _s1(ps=1080*1024*4) /data11/win2_20050831/test01/            -
// ICCUDPPLAY/tl=1080/pause=0.25 _s1 "192.168.1.200" 5000                                       -
// pipe off                                                                                     -
// pipe on                                                                                      -
// iccdearchive pictest _s1(ps=32M) /data11/steswd/                                             -
...
/* Prototypes ---------------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------------------------*/
VOID iccvQuickSortStringLast19( CHR08 **string, INT32 left, INT32 rght);
...
VOID mainroutine()
{
  ...
  archiveSpec[0] = iccArchiveInit(pathName, "tmp", MAXFILES*NDIR);      // spec really big.
                                                                        //
  for(ii=1; ii<nDires; ii++)                                            // Create the other
  {                                                                     // archive specs.
    sprintf(pathName,"%s%s", iDireName[ii].c_str(), iFileName.c_str()); // 
    archiveSpec[ii] = iccArchiveInit(pathName, "tmp", MAXFILES );       //
  }                                                                     //-----------------------
  ...
  {                                                                          // for all possible
    while(iccStringStackPop(archiveSpec[ii]->fileNameStack, &tempString)==0) // directories
    {                                                                        // except the first.
      iccStringStackPush(archiveSpec[ 0]->fileNameStack, tempString);        // Push the
    }                                                                        // filename to the
                                                                             // first spec.
    iccArchiveTerm( archiveSpec[ii] );                                       // Terminate
  }                                                                          // unneeded spec.
  ...
  //---------------------------------------------------------------------------------------------
  iccvQuickSortStringLast19( (CHR08 **)archiveSpec[ 0]->fileNameStack->theStack,
			     0,
			     archiveSpec[0]->fileNameStack->index );

  // Flip stack to prepare for poping.
  //---------------------------------------------------------------------------------------------
  iccStackFlipVOIDP( archiveSpec[ 0]->fileNameStack );
  ...
  for(ii=0; ii<(fileCounter-1); ii++)
  {
    ...
    m_times2str(iHeader.timecode, 0.0, ' ', timeString);
    sprintf(s,"%s",timeString.c_str());
    stackStatus = iccStringStackPop(archiveSpec[0]->fileNameStack, &tempString);
  ...
  while( !Mc->break_ )
  {
    ...
    iccArchiveReport( archiveSpec[0] );
...
VOID iccvQuickSortStringLast19( CHR08 **string, INT32 left, INT32 rght)
{
  ...
  if(left < jj) 
  {
    iccvQuickSortStringLast19(string, left, jj);
  }
  if(ii < rght)
  {
    iccvQuickSortStringLast19(string, ii, rght);
  }
}
