/*-------------------------------UNCLASSIFIED---------------------------------*/
// File: xccNewRate.c
// Desc: This file contains the code for the xccNewRate.
// Hist: When       Who  What
...
//------------------------------------------------------------------------------
// Rout: xccNewRateInit.
// Desc: This routine initializes the specification for xccNewRate.
//------------------------------------------------------------------------------
xccNewRateSpec *xccNewRateInit(FLT32 fs, FLT32 fsNew, INT32 nTaps)
{
  ...
  //----------------------------------------------------------------------------
  spec = (xccNewRateSpec *) xccMallocVOID_Align032(sizeof(xccNewRateSpec));
  ...
  // Allocate buffers.
  //----------------------------------------------------------------------------
  spec->w           = xccMallocFLT64_Align032( spec->nTaps );
  spec->data        = xccMallocFLT64_Align032( spec->nTaps );
  spec->hist        = xccMallocFLT64_Align032( spec->nTaps );

  // Clear buffers.
  //----------------------------------------------------------------------------
  xccvZeroFLT64( spec->w,    1, spec->nTaps );
  xccvZeroFLT64( spec->data, 1, spec->nTaps );
  xccvZeroFLT64( spec->hist, 1, spec->nTaps );
...
//------------------------------------------------------------------------------
// Rout: xccNewRate
// Desc: This routine performs xccNewRate.
//------------------------------------------------------------------------------
INT32 xccNewRate(FLT32 *iData, FLT32 *oData, UNT32 n, xccNewRateSpec *spec)
{
  ...
  // Manage history.
  //----------------------------------------------------------------------------
  xccvCopyFLT32( &iData[n-spec->nTaps], 1, spec->hist, 1, spec->nTaps );
...
//------------------------------------------------------------------------------
// Rout: xccNewRateTerm
// Desc: This routine terminates the specification for xccNewRate.
//------------------------------------------------------------------------------
