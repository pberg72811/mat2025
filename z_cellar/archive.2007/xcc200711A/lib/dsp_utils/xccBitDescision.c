...
{
  xccBitDescisionSpec *spec;

  spec = (xccBitDescisionSpec *) malloc(sizeof(xccBitDescisionSpec));
  ...
  spec->bitStack         = xccStackInitINT08( 1024*64, "BitStack" );

  return( spec );
}

//------------------------------------------------------------------------------
// Rout: xccBitDescisionFLT32
...
VOID xccBitDescisionFLT32(FLT32 *x, INT32 n, xccBitDescisionSpec *spec)
{
  ...
  //----------------------------------------------------------------------------
    if( (spec->clckNew >= 0.0) && (spec->clckOld < 0.0) )
    {
      ...
      {
	spec->dataZeroCrossOne = FALSE;
	//xccOctaveWriteFileFLT32( "debugData", "debugData", x, 1, ii );
	//xccOctaveWriteFileFLT32( "debugClck", "debugClck", clockHist, 1, ii );
      }
    }
  ...
  //----------------------------------------------------------------------------
    if( (spec->clckNew <= 0.0) && (spec->clckOld > 0.0) )
    {
      ...
      if(spec->dataNew > 0) 
      {
	xccStackPushINT08 ( spec->bitStack, 1 );
	printf("Bit = 1\n");
      }
      else
      {            
	xccStackPushINT08 ( spec->bitStack, 0 );
	printf("Bit = 0\n");
      }
    }
  }
