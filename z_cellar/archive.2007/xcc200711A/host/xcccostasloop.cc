...
//                               V                                                              -
//    :-----------:     :------------------:     :-----------:                                  -
//    | File/Pipe |---> | xcccostasloop.cc |---> | File/Pipe |                                  -
//    :-----------:     :------------------:     :-----------:                                  -
//                               |                                                              -
//                               V                                                              -
//                       Debug Statements                                                       -
...
//                                                                                              -
// Examples:                                                                                    -
// xcccostasloop fTADPSOI_DESPREAD ovco oerr ophsi ophsq 25 10 .5 1024                          -
// xcccostasloop ftadpsoi_20041201_022528_t6_short ovco oerr ophsi ophsq 20 10 .5 1024          -
//                                                                                              -
//01.0 Description                                                                              -
//02.0 Requirements                                                                             -
//03.0 Interfaces                                                                               -
...
VOID mainroutine()
{
  ...
  xccVCOSpec        *vcoSpec;
  xccLoopFilterSpec *lpfSpec;

  xccBufferCFLT64_t *circularBufferI;
  xccBufferCFLT64_t *circularBufferQ;
  ...
  iDataCFLT64 = xccMallocCFLT64_Align032( xferSamps );
  oVcoCFLT32  = xccMallocCFLT32_Align032( xferSamps );
  oDroICFLT32 = xccMallocCFLT32_Align032( xferSamps );
  oDroQCFLT32 = xccMallocCFLT32_Align032( xferSamps );
  oErrFLT32   = xccMallocFLT32_Align032 ( xferSamps );

  // xccpll VCO initialization.
  //---------------------------------------------------------------------------------------------
  vcoSpec = xccVCOInit(refFreq, fs, 0.0);

  // xccpll loop filter initialization.
  //---------------------------------------------------------------------------------------------
  lpfSpec = xccLoopFilterInit(lockInRange,       // Lock in range.
  ...
  //---------------------------------------------------------------------------------------------
  circularBufferI  = xccBufferInitCFLT64( integratCount, "Circular Buffer I" );
  circularBufferQ  = xccBufferInitCFLT64( integratCount, "Circular Buffer Q" );
  xccBufferClearCFLT64( circularBufferI );
  xccBufferClearCFLT64( circularBufferQ );
  ...
  while( (nGot==xferSamps) && (!Mc->break_) )
  {
    ...
    for(ii=0; ii<nGot; ii++)
    {
      CFLT64 tempCFLT64;
                                                             ...
                                                             //----------------------------------
      vcoCFLT64 = xccVCOCFLT64( lpfError, vcoSpec );         // Update the VCO.                 -
                                                             //                                 -
      ...
      if(ii==0)                                              //                                 -
      {                                                      //                                 -
	xccBufferWriteCFLT64( circularBufferI, &outICFLT64 );// Write data in circ. buffer.     -
	xccBufferWriteCFLT64( circularBufferQ, &outQCFLT64 );// Write data in circ. buffer.     -
	xccBufferSumCFLT64(circularBufferI);                 // Perform summation.              -
	xccBufferSumCFLT64(circularBufferQ);                 // Perform summation.              -
      }                                                      //                                 -
      else                                                   //                                 -
      {                                                      //                                 -
	xccBufferWriteSumCFLT64( circularBufferI, &outICFLT64 );// WriteSum data in circ. buffer.
	xccBufferWriteSumCFLT64( circularBufferQ, &outQCFLT64 );// WriteSum data in circ. buffer.
      }                                                      //                                 -
                                                             //----------------------------------
      ...
      phase0    = atan2(tempCFLT64.i, tempCFLT64.r);         // Take phase.                    -
                                                             //                                -
      lpfError  = xccLoopFilterFLT64( phase0, lpfSpec );     // Run phase through the loop fltr.
