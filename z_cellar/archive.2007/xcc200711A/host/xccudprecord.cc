...
//                          V                                                                   -
//    :--------:     :---------------:     :---------:                                          -
//    |NIC CARD|---> |xccudprecord.cc|---> |FILE/PIPE|                                          -
//    :--------:     :---------------:     :---------:                                          -
//                          |                                                                   -
//                          V                                                                   -
//                    Debug Statements                                                          -
...
#define BUFFER_SIZE (XCC_MAX_MSG_LEN)
#define MAX_PACKET_SIZE (XCC_MAX_MSG_LEN)
...
VOID mainroutine()
{
  ...
  INT32 loopBeBytes;
  FLT32 loopPercent;
  xccDatagramSocketSpec *datagramSpec;
  ...
  //---------------------------------------------------------------------------------------------
  datagramSpec = xccDatagramSocketReceiveInit(MAX_PACKET_SIZE+1024, thePort);

  // Initialize input buffer.  Do not do this.  The "msg" buffer in the datagramSpec will be 
  //                           the input buffer.
  //---------------------------------------------------------------------------------------------
  // oDataINT08 = xccMallocINT08_Align032(BUFFER_SIZE);  // Leave commented out.

  // Are we non-blocking?
  //---------------------------------------------------------------------------------------------
  if(nonBlockingFlag)
  {
    xccDatagramSocketReceiveSetNB(datagramSpec);
  }
  ...
  if(multicastFlag) 
  {
    xccDatagramSocketReceiveSetMC(datagramSpec, localIP.c_str(), mcastIP.c_str());
  }
  ...
  while(!Mc->break_)
  {
    ...
    // Receive a packet.
    //-------------------------------------------------------------------------------------------
    nBytes = xccDatagramSocketReceive(datagramSpec);
  ...
  m_close(oHeader);
  //xccFree(oDataINT08);  // Not used.
  xccDatagramSocketTerm(datagramSpec);
