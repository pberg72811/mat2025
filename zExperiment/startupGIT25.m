...
%
git25Dir = getenv("USERPROFILE");                % DEFINE GIT FOLDER OF ALL PROJECTS.
git25Dir = strcat(git25Dir,"\GIT25");            %
cd(git25Dir);                                    %

dirList = dir("*2025*");

for ii = 1:length(dirList)                       % GO THROUGH EACH PROJECT AND
    aFolder =...                                 %
      strcat(git25Dir, '\', dirList(ii).name );  %
    disp(aFolder)                                %

    if isfolder(aFolder)                         %
        cd(aFolder);                             %
        if isfile(".\startThis.m")               % RUN SPECIALIZED STARTUP
            run(".\startThis.m");                % SCRIPT.
        end                                      % 
        cd("..")                                 %
    end                                          %
end

dataDir = strcat(git25Dir, '\ZDATA00');          % DATA DIRECTORY ENVIRONMENT VARIABLE.
setenv("DATADIR", dataDir);                      % For reference.
